...
function LeafletMap() {
  ...
  markersRef = L.markerClusterGroup({
    chunkedLoading: true,
  ...
  tileRef = L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
  ...
  const fetchData = async () => {
    ...
    const response = await fetch(`${API_URL}/getLocations/${window.year}`);
    ...
    setMarkerJson(responseJson);
    createMarkers(responseJson);
  ...
  const fetchOnlineCourseData  = async () => {
    ...
    responseJson = await response.json();
    setFilteredOnlineCourses(responseJson.length)
    setOnlineCourses(responseJson);
  }
  ...
  useEffect(() => {
    ...
    setMarkerClusterGroup(markersRef);
  ...
  const filterData = (inputField) => {
    ...
    if (inputField.target.value == "") {
      ...
      var lowercaseInput = inputField.target.value.toLowerCase()
      var filteredMarkers = new Map();
      Object.entries(markerJson).map( ([keyword, array]) => 
        filteredMarkers.set(keyword, array.filter( data => data[2].toLowerCase().search(lowercaseInput) > -1
      )));
      createMarkers(Object.fromEntries(filteredMarkers));

      // filters the amount of online courses for input
      setFilteredOnlineCourses(onlineCourses.filter(data => data[0].toLowerCase().search(lowercaseInput) > -1).length)
    }
    mapInstance.fireEvent("moveend");
  }
...
// is called each time when the map is moved and count the amount of markers
  ...
  function addCategoryLegendToMap() {
