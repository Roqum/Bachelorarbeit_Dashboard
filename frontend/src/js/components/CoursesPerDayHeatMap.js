...
function HeatMap() {
    ...
    const fetchData = async () => {
        const response = await fetch(`${API_URL}/coursesStartDateNoYear/${window.year}`);
        responseJson = await response.json();

        // maps the fetched file in valid format for the charts 
        setFetchedDataset(responseJson.map( startDate => [parseInt(startDate[0].split("-")[0]), parseInt(startDate[0].split("-")[1]), parseInt(startDate[1])]));
    }
