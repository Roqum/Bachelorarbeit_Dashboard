...
function CitiesBarChart() {
    ...
    const ammountOfShownCties = 20; // is the number of cities that will be shown in the chart
    ...
    const fetchData = async () => {
        const response = await fetch(`${API_URL}/getCoursesInCity/${window.year}`);
        responseJson = await response.json();

        // map the fetched file in valid format for the charts 
        setFetchedDataset(responseJson.map(listElem => [listElem[0], parseFloat(listElem[1])]));
    }
