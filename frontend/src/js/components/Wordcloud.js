...
function WordCloudComponent() {
  ...
  function fetchWordCloudData() {
      fetch(`${API_URL}/wordsCount/${window.year}`)
        .then( (res) => res.json())
        .then( (data) => { wordcloudDataRaw = data; 
                          transformDataForCloud(wordcloudDataRaw);
                        })
  ...
  function mouseHoverEventOver(word) {
    var wordCount = 0;
