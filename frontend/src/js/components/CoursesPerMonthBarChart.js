...
function CoursesPerMonthBarChart() {
    ...
    const fetchData = async () => {
        const response = await fetch(`${API_URL}/coursesStartDateNoDay/${window.year}`);
        responseJson = await response.json();
        responseJson = responseJson.map(listElem => [parseFloat(listElem[0]), parseFloat(listElem[1])])
        setMaxValue(Math.max(...[].concat(...responseJson)));

        // map the fetched file in valid format for the charts 
        setStartDateJson(responseJson);
    }
